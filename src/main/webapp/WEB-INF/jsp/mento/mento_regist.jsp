    ...
    <section class="checkout spad mento-regist">
		...
		<div class="container">
			<div style="margin-left:150px">
				<h4>
					안녕하세요 <span style="color: orange; font-weight: 800">${sessionScope.nickname}</span>
					님<br> 지식 공유자가 되기 위해서는 아래의 정보가 필요해요
				</h4>
				<div>
					<hr>
				</div>
				...
				<div class="mentoregist">
					<span>사용자 ID</span> <input type="text" value="${sessionScope.id}"
						readonly v-model="user_id" name="user_id" ref="user_id"> <span
						style="color: red;">{{isok}}</span>
				</div>

				<div class="mentoregist">
					<span>직무 카테고리</span> <select
						style="height: 60px; width: 350px; font-size: 17px"
						v-model="job_cat" ref="job_cat" name="job_cat">
				...
				<div class="mentoregist">
					<span>현재 직업</span> <input type="text" placeholder="현재 직업을 입력해 주세요"
						v-model="job" ref="job" name="job">
				</div>
				<div class="mentoregist">
					<span>멘토링 제목</span> <input type="text"
						placeholder="등록할 멘토링 제목을 입력해주세요" v-model="title" ref="title"
						name="title">
				</div>
				<div class="mentoregist">
					<span style="vertical-align: top;">멘토링 내용</span>
					<textarea rows="10" cols="46"
						placeholder="간단한 자기소개부터 멘토링에 대한 설명을 해주세요" v-model="intro"
						ref="intro" name="intro">
                            	</textarea>
				</div>

				<div class="mentoregist">
					<span>부서</span> <input type="textarea" placeholder="속하신 부서를 적어주세요"
						v-model="dept" ref="dept" name="dept">
				</div>
				<div class="mentoregist">
					<span style="vertical-align: top;">경력 사항</span>
					<textarea rows="10" cols="47" placeholder="경력사항을 작성해주세요"
						v-model="career" ref="career" name="career">
                            	</textarea>
				</div>
				...
				<div class="mentoregist">
					<span>시간당 희망금액<br>(15,000이하)
						...
						step="2000" v-model="cost" name="cost" ref="cost">
				</div>

				<div style="margin-bottom: 50px">
					<div style="text-align: center;">개인정보 활용동의, 수익에 대한 약관 동의</div>
					<label class="text-center" style="display: block"> <input
						type="checkbox" value="agree" ref="agree" v-model="agree">
						동의하기
					</label>
				</div>
				...
				<div class="text-center">
					<span class="button-mento boxhover"
						v-on:click="isok !== '등록된 계정입니다.' && submitForm()"> 등록하기</span> <span
						class="button-cancle boxhover" v-on:click="back()">취소</span>
				</div>
